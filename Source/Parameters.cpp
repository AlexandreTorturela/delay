...
static juce::String stringFromMillisecond(float value, int) {
	if(value<10.0f)
...
static float hzFromString(const juce::String& text) {
	...
	if (!text.endsWithIgnoreCase("Hz")) {
		if (text.endsWithIgnoreCase("kHz") || value < 20.0f) {
			return value * 1000.0f;
		}
	}
	return value;
...
juce::AudioProcessorValueTreeState::ParameterLayout Parameters::createParameterLayout()
{
	...
	layout.add(std::make_unique<juce::AudioParameterFloat>(
		...
		juce::AudioParameterFloatAttributes()
			.withStringFromValueFunction(stringFromMillisecond)
	...
	juce::StringArray noteLengths = { "1/32", "1/16 trip", "1/32 dot", "1/16", "1/8 trip", "1/16 dot", "1/8",
		"1/4 trip", "1/8 dot", "1/4", "1/2 trip", "1/4 dot", "1/2", "1/1 trip", "1/2 dot", "1/1" };
...
{
	double durationInSeconds = 0.02;
	gainSmoother.reset(sampleRate, durationInSeconds);
	coeff = 1.0f - std::exp(-1.0f / (0.2 * float(sampleRate)));
	...
	lowCutSmoother.reset(sampleRate, durationInSeconds);
	highCutSmoother.reset(sampleRate, durationInSeconds);
}
